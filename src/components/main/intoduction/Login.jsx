...
function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    // Əgər email və ya password daxil edilməyibsə
    if (!email || !password) {
      setError("Both fields are required.");
      return;
    }

    // Əgər email düzgün yazılmayıbsa
    if (!emailRegex.test(email)) {
      setError("Please enter a valid email address.");
      return;
    }

    // Əgər password 6-dan azdırsa error mesajı göstəririk
    if (password.length < 6) {
      setError("Password must be at least 6 characters.");
      return;
    }

    // Əgər hər şey düzgündürsə, erroru təmizləyirik
    setError("");

    // Yönləndirmə əmri: login səhifəsindən uğurlu giriş sonrası hesab səhifəsinə yönləndiririk
    navigate("/account");
  };

  const isValid = !error && emailRegex.test(email) && password.length >= 6;
  ...
  return (
    <main>
      ...
      <div className="bg-[#0000009c] h-[580px] max-w-[450px] mx-auto p-12 my-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="relative">
              <input
                ...
                } peer focus:outline-2 outline-offset-2 h-[55px] pl-3 rounded-[.3rem] bg-[#191919b2] text-white border-[#dddddd4c] border-[1px] w-full 
            ...
            <button
              type="submit"
              className="bg-red-600 flex justify-center items-center h-[42px] font-semibold text-white rounded-md"
            >
              Sign in
            </button>
          </form>
